...
def load_data(*args, **kwargs):
    ...
    while True:
        try:
            ...
            customers_page = response.get('QueryResponse', {}).get('Customer', [])
            if not customers_page:
                print(f"No se encontraron datos en el")
                break
            print(f"Procesando pagina {pages_read+1}")

            #Tenemos que acceder a MetaData para accerder a una fecha y filtar los customers
        
            filtered_customers = []
            for customer in customers_page:
                create_time_str = customer.get("MetaData", {}).get("CreateTime", "")
                if create_time_str:
                    ...
                    if start_date <= create_time <= end_date:
                        print(f"Procesando fecha {create_time}")
            ...
            pages_read += 1
            rows_act += len(filtered_customers)            
            # METADATOS
            for customer in filtered_customers:
                all_customers.append({
                    ...
                    'request_customer': query
                })
            
            if len(customers_page) < page_size:
    ...
    df = pandas.DataFrame(all_customers)
    if all_customers:
